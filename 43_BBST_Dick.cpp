...
private:
	struct BinaryNode {
		...
		BinaryNode(string data) : data(data) { } // was int.
	...
	///////////////////////////
	bool search(string target, BinaryNode* node) { // was int
		if(!node)
			return false;
	...
	// slower code than dr's, different idea too.
	bool prefix_exist(string target, BinaryNode* node) { // was int
		if(!node)
			return false;

		if(node->data.size() >= target.size()) {
			bool flag = true;
			for(int prefix_length = 0; prefix_length < target.size(); prefix_length++) {
				if(target[prefix_length] != node->data[prefix_length]) {
					flag = false;
					break;
				}
			}
	...
	BinaryNode* delete_node(string target, BinaryNode* node) { // was int
		if (!node)
			return nullptr;
...
public:
	void insert_string(string target) { // was int.
		if (!root)
	...
	void delete_value(string target) { // was int
		if (root) {
	...
	bool word_exist(string target) { // was int, was search
		return search(target, root);
	}

	bool prefix_exist(string target) { // new function
		return prefix_exist(target, root);
	}
...
void f1() {
	// all is O(lognk), where k is lenght of words.
	AVLTree tree;
	...
	// cout << tree.prefix_exist("abcd") << '\n';
}

void f2() {
	AVLTree tree;
	tree.insert_string("aaa");
	tree.insert_string("aab");
	tree.insert_string("aba");
	tree.insert_string("abb");
	tree.insert_string("baa");
	tree.insert_string("bab");

	tree.insert_string("caa");
	tree.insert_string("cba");
	// looks good.
	tree.print_inorder();
